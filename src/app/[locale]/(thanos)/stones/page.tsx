...
export async function generateMetadata({
  ...
  const localized = getThanosLocalizedCoreCopy(locale, 'stones');
...
export default async function StonesPage({ params }: { params: Promise<{ locale: Locale }> }) {
  const { locale } = await params;
  if (locale === 'ru') return <ThanosLocalizedCorePage locale={locale} pageKey="stones" />;
